...
const App: React.FC = () => {
  ...
  // Master Data: Integrantes
  const [integrantes, setIntegrantes] = useState<Integrant[]>([]);

  // Global App Loading (Auth check)
  const [loading, setLoading] = useState(true);
  
  // Internal Data Loading (Fetching requests)
  ...
  useEffect(() => {
    const init = async () => {
      try {
        // Load Integrantes First (or parallel)
        ...
        if (email) {
            ...
            // If no email returned (external fetch), stop loading and show login screen
            setLoading(false);
        }
      } catch (err) {
  ...
  // Polling / Auto-Refresh Hook
  useEffect(() => {
    if (!userEmail) return;

    const intervalId = setInterval(async () => {
      setIsSyncing(true);
      await fetchRequests(userEmail, role === UserRole.ANALYST, true); // Silent fetch
      
      // Also refresh integrantes occasionally? Not strictly needed every 15s but ok.
      // const freshIntegrantes = await gasService.getIntegrantesData();
  ...
  const determineUserName = (email: string, currentIntegrantes: Integrant[] = []) => {
      ...
      if (found) {
          // Capitalize Name
          return `Hola, ${found.name.toUpperCase()}`;
      }
  ...
  const handleLoginSuccess = async (email: string, auto: boolean = false, preloadedIntegrantes?: Integrant[]) => {
      ...
      // 3. Fetch Data with visual indicator inside the dashboard
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
        ...
        <div className="bg-white p-8 rounded-lg shadow-2xl text-center max-w-md w-full border-t-8 border-brand-red">
            ...
            <div className="mb-8">
               ...
               <h3 className="text-xl font-bold text-brand-red tracking-wide">
            ...
            <div className="text-left mb-6">
                ...
                <input 
                    ...
                    className="w-full bg-gray-50 text-gray-900 placeholder-gray-400 border border-gray-300 rounded-md shadow-sm p-3 focus:ring-2 focus:ring-brand-red focus:border-brand-red outline-none transition"
            ...
            <div className="space-y-3">
                <button 
                    ...
                    className="w-full bg-brand-red text-white py-3 px-4 rounded font-bold uppercase tracking-wide hover:bg-red-700 transition shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                >
                    INGRESAR
                </button>
                ...
                <div className="relative flex py-2 items-center">
                    ...
                    <span className="flex-shrink-0 mx-4 text-gray-400 text-xs uppercase font-bold">O Acceso Staff</span>
                    <div className="flex-grow border-t border-gray-300"></div>
                </div>
      ...
      {role === UserRole.REQUESTER && view === 'LIST' && (
        <div className="space-y-6">
           <div className="flex justify-between items-center border-b pb-4 border-gray-200">
             ...
             <button 
               onClick={() => setView('NEW')}
               className="bg-brand-red text-white px-5 py-2 rounded shadow hover:bg-red-700 transition font-bold uppercase text-xs tracking-wide"
             >
               + Nueva Solicitud
             </button>
           </div>
                   ...
                   <div key={req.requestId} className="bg-white p-6 rounded shadow-sm hover:shadow-md transition border-t-4 border-brand-red relative group">
                     ...
                     <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                        ...
                        <button 
                            ...
                            className="text-brand-red text-xs font-bold uppercase tracking-wide hover:underline focus:outline-none"
                        >
                            Ver Detalle
                        </button>
                     </div>
               ...
               {requests.length === 0 && (
                 <div className="col-span-full text-center py-16 bg-white rounded border border-gray-200 flex flex-col items-center justify-center">
                   ...
                   <p className="text-gray-500 mb-6 max-w-sm text-sm">
                     Parece que aún no has registrado ningún viaje. ¡Crea tu primera solicitud ahora para comenzar el proceso!
                   </p>
                   <button 
                     onClick={() => setView('NEW')}
                     className="bg-brand-red text-white px-5 py-2.5 rounded font-bold shadow hover:bg-red-700 transition uppercase text-xs tracking-wide"
                   >
                     Crear Solicitud
                   </button>
                 </div>
