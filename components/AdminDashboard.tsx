...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ requests, onRefresh, isLoading, onViewRequest }) => {
  ...
  const confirmFinalize = (req: TravelRequest) => {
    setDialog({
        ...
        message: `¿Está seguro de cerrar la solicitud ${req.requestId}?\n\nEsto indicará que el proceso ha concluido y no se podrán cargar más soportes.`,
        type: 'CONFIRM',
                    ...
                    <>
                      {filteredRequests.map((req) => (
                        <tr key={req.requestId}>
                          <td className="whitespace-nowrap px-3 py-4 text-sm font-bold text-gray-900">{req.requestId}</td>
                          ...
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                            {req.departureDate}<br/>
                            {req.returnDate ? req.returnDate : <span className="text-xs italic text-gray-400">Solo Ida</span>}
                          </td>
                          ...
                          <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <div className="flex justify-end gap-2 items-center">
                              ...
                              {/* SUPPORTS LOGIC */}
                              {(req.status === RequestStatus.APPROVED || req.status === RequestStatus.PROCESSED) && (
                                <>
                                    {/* Upload/View Button */}
                                    <button 
                                      onClick={() => setSelectedRequestForSupports(req)}
                                      className={`text-xs font-bold px-2 py-1 rounded border ${req.status === RequestStatus.APPROVED ? 'text-blue-600 border-blue-200 bg-blue-50 hover:bg-blue-100' : 'text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                                    >
                                      {req.status === RequestStatus.APPROVED ? (req.supportData?.files?.length ? 'Cargar Más' : 'Cargar Soportes') : 'Ver Soportes'}
                                    </button>

                                    {/* Finalize Button (Only if Approved and has at least one file) */}
                                    {req.status === RequestStatus.APPROVED && req.supportData && req.supportData.files.length > 0 && (
                                        <button 
